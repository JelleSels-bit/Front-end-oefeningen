import Countries, {type CountryItem} from 'countries-capitals'
import {randomIntFromInterval, shuffle} from './utils.js'
import {type IQuestion} from '@/models/IQuestion.ts'
...
export const getQuestions = (region: string | undefined, numberOfQuestions: number): IQuestion[] => {
    ...
    const countriesInRegion = shuffle<CountryItem & {city: string}>(
      (new Countries().byLocation(region)['countries'] as CountryItem[]).filter(c => c.city !== null) as (CountryItem & {
          city: string
      })[],
    )
...
export const getMaxNumberOfQuestionsForRegion = (region: string): number => {
    const countries = new Countries().byLocation(region)['countries'] as CountryItem[]
    return countries.filter((c: CountryItem) => c.city).length
}
...
const getAllRegionsSet = (minSize = 5): Set<string> => {
    ...
    return new Set(
      countries
        ...
        .map(c => c.location)
        .filter(l => l)
        .filter(l => (new Countries().byLocation(l)['countries'] as CountryItem[]).filter(c => c.city).length >= minSize),
    ) as Set<string>
}
 ...
 */
const getAlternatives = (countries: CountryItem[], i: number): string[] => {
